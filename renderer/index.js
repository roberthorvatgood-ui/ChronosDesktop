// [Updated 2026-01-30 22:17 CET] Renderer — File click uses app downloader; Download Selected sends /exp/...; ZIP=/zip?date
(function(){
  ...
  function setStatus(state,text){ currentStatusState=state; el.status.className='chip status-'+state; if(!text){ text= state==='on'? `${t('toast.connected','Connected')} (${Store.getBase()||''})` : state==='checking'? t('status.checking','Chronos: Checking…') : t('status.off','Chronos: Not available'); } el.status.textContent=text; }
  ...
  async function loadStatus(base){ try{ const s=await window.chronos.status(base, Store.getPaths()); if(!s||!s.ok) throw 0; el.totalText.textContent=bytes(s.total); el.usedText.textContent=bytes(s.used); const pct=Math.max(0,Math.min(100,Math.round((s.used/Math.max(1,s.total))*100))); el.usedPct.textContent=pct+'%'; el.meterFill.style.width=pct+'%'; if(el.diagBaseText) el.diagBaseText.textContent=base; if(el.diagStatusText) el.diagStatusText.textContent=t('toast.connected','Connected'); if(el.diagStorageText) el.diagStorageText.textContent=`${el.usedText.textContent} / ${el.totalText.textContent} (${el.usedPct.textContent})`; if(el.diagTimeText) el.diagTimeText.textContent=nowText(); setStatus('on'); } catch{ el.totalText.textContent='–'; el.usedText.textContent='–'; el.usedPct.textContent='0%'; el.meterFill.style.width='0%'; setStatus('off'); } }

  async function loadList(base){ el.listSkeleton.classList.remove('hidden'); try{ const data=await window.chronos.list(base, Store.getPaths()); const dates=Array.isArray(data&&data.dates)?data.dates:[]; rawDays = dates.map(d=>({date:d.date, files:Array.isArray(d.files)?d.files:[]})); el.itemsCount.textContent= rawDays.reduce((a,d)=>a+d.files.length,0); el.daysCount.textContent= rawDays.length; renderList(); } finally { el.listSkeleton.classList.add('hidden'); } }

  function renderList(){
    const q=(el.search.value||'').trim().toLowerCase(); const sort=el.sort.value||'date-desc';
    const matchFile=f=>{ if(!q) return true; const name=(f.name||'').toLowerCase(); const mode=(f.mode||'').toLowerCase(); const bytesStr=String(f.bytes??'').toLowerCase(); return name.includes(q)||mode.includes(q)||bytesStr.includes(q); };
    ...
    el.list.textContent='';
    if(!days.length){ const wrap=document.createElement('div'); wrap.className='card'; wrap.style.cssText='text-align:center;padding:24px'; const t1=document.createElement('div'); t1.style.fontWeight='700'; t1.style.marginBottom='6px'; t1.textContent=t('list.empty','No data.'); const t2=document.createElement('div'); t2.className='muted'; t2.textContent='Try Refresh, adjust filters, or check your device URL.'; wrap.appendChild(t1); wrap.appendChild(t2); el.list.appendChild(wrap); return; }
    ...
    for(const d of days){
      ...
      // ZIP per day (old working route)
      const zip=document.createElement('a'); zip.className='btn ghost'; zip.innerHTML='<span class="i i-arrow-download"></span><span>ZIP</span>';
      try{ zip.href = new URL(`/zip?date=${encodeURIComponent(d.date)}`, Store.getBase()).toString(); }catch{ zip.remove(); }
      ...
      toggle.innerHTML = (expandState[d.date] ? '<span class="i i-chevron-up"></span><span>Collapse</span>' : '<span class="i i-chevron-down"></span><span>Expand</span>');
      ...
      if(expandState[d.date]){
        ...
        for(const f of d.files){
          ...
          // Selection key: store /exp/<date>/<name> (old style) for reliable bulk downloads
          ...
          // Link (click) uses app downloader immediately (streams via main), not external browser
          const link=document.createElement('a'); link.className='file-name'; link.textContent=name||'(unnamed)';
          link.href = pathExp; // for context menu copy; real download goes through IPC below
          link.addEventListener('click', async (e)=>{
            ...
            try{
              ...
              if(r && r.ok) showToast('Download started'); else showToast('Download failed');
          ...
          const del=document.createElement('button'); del.className='btn danger'; del.innerHTML='<span class="i i-delete"></span><span>Delete</span>';
          del.addEventListener('click', async e=>{ e.stopPropagation(); const ok=confirm(`Delete file: ${name}?`); if(!ok) return; try{ const r=await window.chronos.rmFile(Store.getBase(), pathExp); if(r&&r.ok){ showToast('Deleted'); await loadList(Store.getBase()); } }catch(err){ console.error(err); } });
  ...
  // Actions
  el.save?.addEventListener('click', async ()=>{ const base=(el.base.value||'').trim() || 'http://192.168.4.1'; Store.setBase(base); setStatus('checking'); try{ const v=await window.chronos.version(base, Store.getPaths()); if(v&&v.ok){ setStatus('on'); if(el.diagVersionText) el.diagVersionText.textContent=v.version||''; await loadStatus(base); await loadList(base); showToast('Connected'); } else throw 0; }catch{ try{ const d=await window.chronos.discover(base); if(d && d.ok){ const p={ versionPath:d.hits.version?.path||'/api/version', statusPath:d.hits.status?.path||'/api/status', listPath:d.hits.list?.path||'/api/list' }; Store.setPaths(p); const v2=await window.chronos.version(base,p); if(v2&&v2.ok){ setStatus('on'); if(el.diagVersionText) el.diagVersionText.textContent=v2.version||''; await loadStatus(base); await loadList(base); showToast('Connected'); return; } } }catch{} setStatus('off'); } });
  el.refresh?.addEventListener('click', async ()=>{ const base=Store.getBase() || (el.base.value||'').trim() || 'http://192.168.4.1'; setStatus('checking'); try{ const v=await window.chronos.version(base, Store.getPaths()); if(v&&v.ok){ setStatus('on'); if(el.diagVersionText) el.diagVersionText.textContent=v.version||''; await loadStatus(base); await loadList(base); } else throw 0; }catch{ setStatus('off'); } });
  ...
  el.diagRefresh?.addEventListener('click', async ()=>{ const base=Store.getBase() || 'http://192.168.4.1'; setStatus('checking'); try{ const v=await window.chronos.version(base, Store.getPaths()); if(v&&v.ok){ setStatus('on'); if(el.diagVersionText) el.diagVersionText.textContent=v.version||''; await loadStatus(base); await loadList(base); } else setStatus('off'); }catch{ setStatus('off'); } });
  el.diagCopy?.addEventListener('click', async ()=>{ const text=`Base: ${Store.getBase()||''}
...
Checked: ${(el.diagTimeText&&el.diagTimeText.textContent)||''}`; try{ await navigator.clipboard.writeText(text); }catch{ window.chronos.copyText(text); } showToast('Link copied'); });

  if (window.chronos?.onDownloadProgress) { window.chronos.onDownloadProgress(({id,received,total})=>{ el.progress.classList.add('is-visible'); const pct= total>0?Math.round(received/total*100):0; el.progressFill.style.width=pct+'%'; el.progressLabel.textContent = `Preparing… (${id}) — ${pct}%`; }); }
  if (window.chronos?.onDownloadStep) { window.chronos.onDownloadStep(({done,total,label})=>{ el.progress.classList.add('is-visible'); const pct=Math.round(done/Math.max(1,total)*100); el.progressFill.style.width=pct+'%'; el.progressLabel.textContent = `Saved: ${label} — ${done}/${total}`; if(done>=total){ setTimeout(()=>{ el.progress.classList.remove('is-visible'); el.progressFill.style.width='0%'; el.progressLabel.textContent=''; showToast('Downloads completed'); },900);} }); }

  (async function init(){ setTheme(Store.getTheme()); document.body.classList.toggle('density-compact', Store.getDensity()==='compact'); const base=Store.getBase() || 'http://192.168.4.1'; if(el.base) el.base.value=base; if(el.lang) el.lang.value=Store.getLang(); await loadLocale(Store.getLang()); applyI18n(); setStatus('checking'); try{ const v=await window.chronos.version(base, Store.getPaths()); if(v&&v.ok){ setStatus('on'); if(el.diagVersionText) el.diagVersionText.textContent=v.version||''; if(el.diagBaseText) el.diagBaseText.textContent=base; await loadStatus(base); await loadList(base); } else throw 0; }catch{ setStatus('off'); } })();
})();
