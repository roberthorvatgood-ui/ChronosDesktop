// [2026-01-29 15:20 CET] Renderer v1.2.9 — context menu + multi-select + i18n + theming tokens
(function(){
  const el = mapIds([
    ...
    'tabExports','tabDiag','viewExports','viewDiag','diagBase','diagRun','diagSummary','diagV','diagS','diagL','diagW','wifiList','themeToggle','toast','lang','ctx'
  ]);
  function mapIds(ids){ const o={}; ids.forEach(id=> o[id]=document.getElementById(id)); return o; }

  // Theme overrides via tokens
  try{
    const overrides = JSON.parse(localStorage.getItem('chronos.theme.overrides')||'{}');
    for(const k in overrides){ document.documentElement.style.setProperty(k, overrides[k]); }
  }catch{}
  ...
  // MDN recommends Intl with navigator.language for localized formatting. [9](https://developer.mozilla.org/en-US/docs/Web/API/Navigator/language)
  ...
  function setStatus(state, text){
    ...
    el.status.textContent = text || (state==='on'? `Chronos: Connected (${Store.getBase()||''})` : state==='checking' ? t('status.checking','Chronos: Checking…') : t('status.off','Chronos: Not available'));
  }
  function showToast(msg){ el.toast.textContent=msg; el.toast.classList.remove('hidden'); setTimeout(()=> el.toast.classList.add('hidden'), 1600); }

  // Data & selection
  let rawDays = [];         // {date, files[]}
  let expandState = {};     // date->bool
  let selectedDays = new Set();
  let selectedFiles = new Set();  // /exp/yyyy-mm-dd/name
  ...
  function renderList(){
    ...
    for(const d of days){
      ...
      left.appendChild(dayTitle); left.appendChild(daySub);

      const right = div('day-actions');
      const chk = document.createElement('input'); chk.type='checkbox'; chk.checked = selectedDays.has(d.date);
      chk.addEventListener('change', ()=>{ if(chk.checked){ selectedDays.add(d.date); } else { selectedDays.delete(d.date); } updateSelectedBadge(); });
      const btnZip = aBtn('btn ghost','ZIP'); window.chronos.zipUrl(Store.getBase(), d.date).then(u=> btnZip.href=u);
      ...
      right.appendChild(chk); right.appendChild(btnZip); right.appendChild(toggle);
      ...
      if(expandState[d.date]){
        ...
        for(const f of d.files){
          ...
          const link = aBtn('file-name', name||'(unnamed)'); window.chronos.dlUrl(Store.getBase(), d.date, name).then(u=> link.href=u);
          const meta = span('file-meta', ` ${(f.mode||'')} • ${(f.size||'')} • ${(f.bytes||0)} bytes`);
          leftF.appendChild(link); leftF.appendChild(meta);
          ...
          del.addEventListener('click', async()=>{
            ...
            try{ const r=await window.chronos.rmFile(Store.getBase(), fp); if(r&&r.ok){ showToast(t('toast.deleted','Deleted')); await loadList(Store.getBase()); } }catch(e){ console.error(e); }
          });

          // Context menu (Open/Copy/Delete)
          row.addEventListener('contextmenu',(ev)=> showCtx(ev, {url:link.href, name, fp, date:d.date}));

          rightF.appendChild(chkF); rightF.appendChild(del);
  ...
  function showCtx(ev, {url, name, fp}){ ev.preventDefault();
    ...
    el.ctx.onclick = async (e)=>{
      ...
      if(act==='open'){ if(url) await window.chronos.openExternal(url); } // opens in default browser [3](https://www.electronjs.org/docs/latest/api/shell)
      else if(act==='copy'){ if(url){ try{ await navigator.clipboard.writeText(url);}catch{ window.chronos.copyText(url);} showToast(t('toast.copied','Link copied')); } } // Clipboard via navigator or preload fallback. [4](https://www.electronjs.org/docs/latest/api/clipboard)
      else if(act==='delete'){
  ...
  el.diagRun?.addEventListener('click', async()=>{
    ...
    try{
      ...
      nets.forEach(n=>{
        const item=div('wifi-item'); const label=div(null,n.ssid); const bar=div('wifi-bar'); const fill=div('wifi-fill'); fill.style.width = Math.max(0,Math.min(100,n.signalPct||0))+'%';
        bar.appendChild(fill); item.appendChild(label); item.appendChild(bar); el.wifiList.appendChild(item);
      });
  ...
  (async function init(){
    ...
    // Language persistence; initialize <select> to storage (fallback navigator.language)
    const lang = Store.getLang(); if(el.lang){ el.lang.value = lang; }
    await loadI18n(lang);
